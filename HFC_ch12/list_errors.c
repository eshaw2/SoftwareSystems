...
Node *make_something() {
    ...
    int val = pop(&node1); // At this point, there are 32 bytes lost
    free(node1);
    push(&node2, val); // Now a total of 48 bytes lost
    node3->next = node2; // 16 bytes lost
    // printf("node 1: %i \n", node1->val);
